...
class ConnectionService : Service() {
    ...
    private fun startForegroundForCurrentState() {
        if (Build.VERSION.SDK_INT >= 29) {
            // Connection-only foreground service. Camera/microphone foreground-service
            // types are added only when the user has actively started those features.
            startForeground(
                NOTIFICATION_ID,
    ...
    private fun startMicrophone() {
        ...
        try {
            val sampleRate = HIGH_QUALITY_SAMPLE_RATE
            ...
            val chunkSize = sampleRate / 50 * 2 // 20 ms PCM chunks
            val bufferSize = max(minBuffer, chunkSize * 4)
            val audioSource = when (requestedMicSource) {
            ...
            audioRecord = try {
                AudioRecord(audioSource, sampleRate, channel, encoding, bufferSize)
            } catch (_: Exception) {
                AudioRecord(MediaRecorder.AudioSource.MIC, sampleRate, channel, encoding, bufferSize)
            }
